...
dataset_root = fullfile(get_root_cnnimageretrieval(), '../../7Scenes');
...
result_dir = fullfile(data_root, '7scenes', sprintf('vgg-%s-%d-desc', desc_tp, test_imdim));
...
for d = 1:numel(test_datasets)
    ...
    [train_im, n] = get_imlist(dataset_root, dataset, 'dataset_train.txt'); 
    ...
    % Extract training images descriptors
	[test_im, n] = get_imlist(dataset_root, dataset, 'dataset_test.txt');
